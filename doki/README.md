...
각 데이터는 `.txt` 파일로 구성되어 있고, 한 줄에는 여러 개의 값이 포함된다.<br>
한 줄은 r또는 s로 시작하는데,
- **r로 시작하는 줄은 실제 좌표 데이터**  
- **s로 시작하는 줄은 쓰레기 데이터**(제외해야 함)
...
- 카메라 시점 조정 코드 → ax.view_init(elev=25, azim=40)     # 보기 좋은 각도(elev=25, azim=40) (elev : z축으로의 시점 변화, azim : x,y 평면으로의 시점 변화)
...
### 왜 필요한가?
- 전체 데이터 개수가 적다. (raw_data는 다 합쳐도 62개)
...
#### 1) Moving Average (스무딩) + IQR 클리핑 (이상치 제거)
- 연속된 값들의 평균을 내서 노이즈가 심한 궤적을 부드럽게 만든다.<br>
...
#### 5) Scale (확대/축소) + Translate (평행 이동)
- 궤적 전체의 크기를 조금 키우거나 줄인다.<br>
